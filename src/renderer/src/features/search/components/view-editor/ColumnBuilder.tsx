...
import { DATA_FIELDS, FIELD_ICONS, TABLE_COLORS, TYPE_LABELS } from '../../../../constants';
import { getBadgeColor, setBadgeAccentCache } from '../../../../utils/searchHelpers';
...
const ColumnBuilder: FC<ColumnBuilderProps> = ({ columns, onColumnsChange }) => {
  ...
  const updateColumnTemplate = (
    id: string,
    templateUpdates: Partial<NonNullable<TableColumn['template']>>,
  ) => {
    onColumnsChange(
      columns.map((c) =>
  ...
  const inputClass =
    'h-10 px-3 rounded-md bg-input-background border border-border text-sm text-foreground placeholder:text-muted-foreground/40 outline-none transition-colors focus:border-primary/50';
            ...
            {filteredFields.map((field) => {
              const Icon = FIELD_ICONS[field.key]
                ? (() => {
                    ...
                    return iconMap[FIELD_ICONS[field.key]] || Database;
                  })()
                : Database;
              ...
              return (
                ...
                >
                  ...
                  {isAlreadyAdded ? (
                    <span className="text-[10px] text-muted-foreground/40 font-bold shrink-0">
                      Added
                    </span>
                  ) : (
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <CustomSelect
                            ...
                            onChange={(val) =>
                              updateColumnTemplate(col.id, { operator: (val as any) || undefined })
                            }
                          ...
                          {col.template?.operator && (
                            <input
                              ...
                              onChange={(e) =>
                                updateColumnTemplate(col.id, { value: e.target.value })
                              }
                              onKeyDown={(e) => {
                                ...
                                {col.template.values.map((val, idx) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <button
                                        onClick={() => {
                                          const newValues = (col.template?.values || []).filter(
                                            (_, i) => i !== idx,
                                          );
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1.5">
                          <button
                            onClick={() =>
                              updateColumnTemplate(col.id, {
                                sortOrder: col.template?.sortOrder === 'asc' ? 'none' : 'asc',
                              })
                            }
                            className={cn(
                          ...
                          <button
                            onClick={() =>
                              updateColumnTemplate(col.id, {
                                sortOrder: col.template?.sortOrder === 'desc' ? 'none' : 'desc',
                              })
                            }
                            className={cn(
                              'px-3 h-8 rounded-md text-xs font-bold border transition-all',
