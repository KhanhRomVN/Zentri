...
const OverviewTab: FC<OverviewTabProps> = ({ device }) => {
  ...
  const rows = [
    { icon: TypeIcon, label: 'Type', value: `${device.type === 'mobile' ? 'Mobile' : 'Desktop'}${device.isVirtual ? ' · Virtual' : ' · Physical'}` },
    { icon: Cpu, label: 'Platform', value: `${device.platform || '—'} ${device.osVersion ? `(${device.osVersion})` : ''}` },
    { icon: Battery, label: 'Battery', value: device.battery != null ? `${device.battery}%` : '—' },
    { icon: HardDrive, label: 'Storage', value: `${formatBytes(device.storageUsed)} / ${formatBytes(device.storageTotal)}` },
    { icon: MemoryStick, label: 'RAM', value: `${formatBytes(device.ramUsed)} / ${formatBytes(device.ramTotal)}` },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {rows.map((row) => (
          <div key={row.label} className="flex items-center gap-2.5 rounded-md border border-border bg-panel/50 p-3">
      ...
      {device.tags && device.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {device.tags.map((tag) => (
            <span key={tag} className="rounded-full border border-border bg-panel px-2.5 py-1 text-[11px] text-text-secondary">
              {tag}
            </span>
          ))}
        </div>
