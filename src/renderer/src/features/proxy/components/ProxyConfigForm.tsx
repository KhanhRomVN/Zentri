...
const inputClass =
  'w-full h-11 px-3 rounded-xl bg-input-background border border-border text-sm text-foreground placeholder:text-muted-foreground/40 outline-none focus:border-primary/50';
  ...
  const handleSave = async () => {
    ...
    try {
      const finalData = {
        ...
        expiredAt:
          formData.expiredAt ||
          new Date(Date.now() + (formData.durationDays || 30) * 24 * 60 * 60 * 1000).toISOString(),
    ...
    <div className="flex flex-col h-full bg-background animate-in fade-in duration-500 overflow-hidden relative border-l border-border">
      <div className="h-[57px] shrink-0 border-b border-border bg-card/10 backdrop-blur-xl px-6 flex items-center justify-between sticky top-0 z-20">
        <h2 className="text-sm font-black uppercase  text-foreground/90 leading-none">
          {proxy ? 'Update Infrastructure' : 'Add New Node'}
        </h2>
      </div>
            ...
            <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-500">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-[10px] font-black uppercase  text-primary/80">
                    Quick Import
                  </span>
                </div>
              ...
              <input
                type="text"
                placeholder="Paste proxy string (host:port:username:password)..."
                value={importString}
                onChange={(e) => {
            ...
            <div className="space-y-5 animate-in slide-in-from-bottom-2 duration-500">
              <div className="space-y-2.5">
                ...
                <input
                  ...
                  value={formData.host || ''}
                  onChange={(e) => setFormData((d: any) => ({ ...d, host: e.target.value }))}
                  className={inputMono}
                />
              </div>
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData((d: any) => ({ ...d, username: e.target.value }))}
                    className={inputMono}
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((d: any) => ({ ...d, purchaseUrl: e.target.value }))
                    }
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2.5">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData((d: any) => ({ ...d, price: parseFloat(e.target.value) || 0 }))
                      }
                      className={inputNoSpinner}
                    />
                  </div>
                          ...
                          {CURRENCY_OPTIONS.map((cur) => (
                            <button
                              ...
                              onMouseDown={() => {
                                setFormData((d: any) => ({
                                  ...d,
                                  metadata: { ...d.metadata, currency: cur },
                                }));
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2.5">
                  ...
                  <input
                    ...
                    value={
                      formData.expiredAt
                        ? new Date(formData.expiredAt).toISOString().slice(0, 16)
                        : ''
                    }
                    onChange={(e) =>
                      setFormData((d: any) => ({
                ...
                <div className="space-y-2.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData((d: any) => ({ ...d, country: e.target.value }))}
                    className={inputClass}
                  />
                </div>
              </div>
      ...
      <div className="h-[75px] shrink-0 border-t border-border bg-card/20 backdrop-blur-xl px-6 flex items-center justify-end gap-3 sticky bottom-0 z-20">
        {proxy && (
          <button
            ...
            className="w-11 h-11 rounded-xl bg-rose-500/10 text-rose-500 border border-rose-500/20 hover:bg-rose-500/20 transition-all active:scale-95 flex items-center justify-center"
          >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-3">
                      ...
                      {[
                        ...
                        { label: 'Location', value: diagnosticResult.direct?.country },
                        { label: 'Region', value: diagnosticResult.direct?.region },
                        { label: 'ISP Provider', value: diagnosticResult.direct?.isp },
                        { label: 'Organization', value: diagnosticResult.direct?.org },
                      ].map((row, i) => (
                        <div
                          key={i}
                          className="flex flex-col p-2.5 bg-muted/5 border border-border/20 rounded-xl"
                        >
                    ...
                    <div className="space-y-3">
                      ...
                      {[
                        ...
                        { label: 'Location', value: diagnosticResult.proxy?.country },
                        { label: 'Region', value: diagnosticResult.proxy?.region },
                        { label: 'ISP Provider', value: diagnosticResult.proxy?.isp },
                        { label: 'Organization', value: diagnosticResult.proxy?.org },
                      ].map((row, i) => (
                        <div
                          key={i}
                          className="flex flex-col p-2.5 bg-primary/5 border border-primary/20 rounded-xl"
                        >
                          ...
                          <span
                            className={cn(
                              ...
                              row.primary ? 'text-primary font-black' : 'text-primary/90',
                            )}
                          >
                            {row.value || 'N/A'}
                          </span>
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-rose-500/[0.03] border border-rose-500/10 rounded-3xl space-y-6">
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-[10px] text-muted-foreground/60 uppercase tracking-wider max-w-[300px] leading-relaxed">
                      {diagnosticResult?.error || 'Unknown Handshake Error'}
                    </p>
                  </div>
                  <button
                    onClick={() => setShowResultModal(false)}
            ...
            <div className="px-6 py-4 space-y-6">
              <div className="flex flex-col items-center justify-center pt-4 pb-2 text-center space-y-4">
                ...
                <p className="text-[13px] text-muted-foreground leading-relaxed">
                  Are you sure you want to decommission this proxy node? This action cannot be
                  undone.
                </p>
              </div>
              <div className="flex gap-3 pt-2">
