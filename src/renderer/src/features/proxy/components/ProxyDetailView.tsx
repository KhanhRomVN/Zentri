...
const DetailItem: FC<{
  ...
  <div
    className={cn(
      'flex flex-col gap-1.5 p-4 rounded-2xl bg-muted/5 border border-border/10 transition-all hover:bg-muted/10',
      className,
    )}
  >
        ...
        <div className="max-w-6xl mx-auto space-y-10">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 px-1">
              ...
              <h3 className="text-[11px] font-black uppercase  text-foreground/50">
                Registry Identity
              </h3>
            </div>
            <div className="grid grid-cols-4 gap-4">
              ...
              <DetailItem
                icon={Lock}
                label="Username"
                value={proxy.username || 'Anonymous'}
                valueClassName="font-mono"
              />
              <DetailItem
                icon={Lock}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <DetailItem
                icon={Shield}
                label="Privacy Tier"
                value={proxy.proxyType === 'private' ? 'EXCLUSIVE' : 'SHARED'}
                valueClassName="text-indigo-400"
              />
              <DetailItem
                icon={Server}
                label="Node Origin"
                value={proxy.sourceType?.toUpperCase()}
                valueClassName="text-emerald-400"
              />
              <DetailItem
                icon={Zap}
                label="Routing Mode"
                value={proxy.rotationType === 'static' ? 'STATIC BOUND' : 'ACTIVE MESH'}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-4">
              <DetailItem
                icon={CreditCard}
                label="Billing Model"
                value={proxy.pricingType === 'time' ? 'TIME-BASED' : 'DATA-BASED'}
              />
              <DetailItem
                icon={CreditCard}
                label="Unit Price"
                value={`${proxy.price || 0} ${proxy.metadata?.currency || 'USD'}`}
              />
              <DetailItem
                icon={Clock}
            ...
            <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm font-bold text-foreground/80 truncate max-w-md">
                    {proxy.purchaseUrl}
                  </span>
                </div>
              </div>
