...
const AccountTable = memo(
  ({
    ...
    const desktopAgents = useMemo(
      () => USER_AGENTS.filter((ua) => ua.group === 'Desktop').slice(0, 5),
      [],
    );
    const mobileAgents = useMemo(
      () => USER_AGENTS.filter((ua) => ua.group === 'Mobile').slice(0, 3),
      [],
    );
            ...
            <div className="w-[180px]">
              <Dropdown className="w-full">
                <DropdownTrigger
                  ...
                  className="w-full h-9 px-2.5 text-xs bg-dropdown-background border border-dropdown-border hover:bg-dropdown-itemHover transition-all rounded-md shadow-sm flex items-center justify-between group"
                >
                  <div className="flex items-center gap-2 truncate flex-1">
                    <Cpu className="w-3.5 h-3.5 text-muted-foreground transition-colors" />
                  ...
                  <ChevronDown className="w-3.5 h-3.5 text-muted-foreground/70 transition-colors ml-1" />
                </DropdownTrigger>
                <DropdownContent
                  align="start"
                  className="w-[220px] max-h-[400px] bg-dropdown-background border-dropdown-border shadow-xl"
                >
                  {/* Existing Agents */}
                  {agents.length > 0 && (
                    <>
                      ...
                      {agents.map((a) => (
                        <DropdownItem
                          ...
                          onClick={(e: React.MouseEvent) => {
                            e.stopPropagation();
                            onUpdateAccountAgent(account.id, a.id);
                          }}
                          className="py-2"
                        >
                          <Tooltip>
