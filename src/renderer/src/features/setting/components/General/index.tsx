            ...
            <div className="flex-1 h-9 rounded-md border border-border bg-input-background px-3 text-sm flex items-center text-muted-foreground overflow-hidden font-mono truncate">
              {isInitializing ? (
                ...
                <span className="truncate">
                  {sqlitePath || 'Failed to detect path'}
                </span>
              )}
            </div>
            <button
              ...
              title={isInitializing ? 'Initializing...' : !sqlitePath ? 'Path not available' : 'Open Folder'}
            >
              <FolderOpen className="w-4 h-4" />
            </button>
          </div>
          <p className="text-[11px] text-muted-foreground/50">
            Automatically managed at <code className="bg-muted px-1 rounded text-foreground">~/.zentri/zentri.sql</code>.
            Profiles are stored in <code className="bg-muted px-1 rounded text-foreground">~/.zentri/profiles/</code>.
          </p>
        </div>
      </div>
    </div>
