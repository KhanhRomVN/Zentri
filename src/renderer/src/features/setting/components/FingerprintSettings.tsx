...
const inputClass = 'w-full h-10 px-3 rounded-xl bg-input-background border border-border text-sm text-foreground placeholder:text-muted-foreground/40 outline-none focus:border-primary/50 font-bold font-mono';
const inputReadOnly = cn(inputClass, 'bg-muted/30 opacity-80 cursor-not-allowed');

const FormSection = ({ title, description, icon: Icon, showHelp = false, isUnlocked = true, children }: { title: string; description?: string; icon: any; showHelp?: boolean; isUnlocked?: boolean; children: React.ReactNode }) => {
...
const StepNavigation = ({ step, onPrev, onNext, isLast = false }: { step: number; onPrev: () => void; onNext: () => void; isLast?: boolean }) => (
  <div className="flex justify-end gap-3 mt-6">
    {step > 1 && <button onClick={onPrev} className="px-6 h-10 rounded-xl bg-muted/10 text-muted-foreground text-[10px] font-black uppercase tracking-widest hover:bg-muted/20 transition-all border border-border/10">Quay lại</button>}
    {!isLast && <button onClick={onNext} className="px-10 h-10 rounded-xl bg-primary/20 text-primary text-[10px] font-black uppercase tracking-[0.25em] hover:bg-primary/30 transition-all">Bước tiếp theo</button>}
  </div>
);

const FormItem = ({ label, description, tooltip, action, children }: { label: string; description: string; tooltip?: React.ReactNode; action?: React.ReactNode; children: React.ReactNode }) => {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-0.5 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <button type="button" onClick={() => setShowTooltip(!showTooltip)} className={cn('p-0.5 rounded-full transition-colors shrink-0', showTooltip ? 'text-primary bg-primary/10' : 'text-muted-foreground/40 hover:text-primary')}><HelpIcon className="w-3 h-3" /></button>
          </div>
          <p className="text-[11px] text-muted-foreground/70 leading-relaxed line-clamp-1">{description}</p>
        </div>
      </div>
      {showTooltip && <><div className="fixed inset-0 z-40" onClick={() => setShowTooltip(false)} /><div className="absolute z-50 left-0 top-10 w-80 p-4 rounded-2xl bg-[#121214] border border-border/50 shadow-2xl animate-in fade-in zoom-in-95 duration-200"><div className="text-[12px] leading-relaxed text-muted-foreground">{tooltip}</div></div></>}
...
// Simple native combobox replacement
const NativeCombobox: React.FC<{ value: string; options: { label: string; value: string }[]; onChange: (val: string) => void; placeholder?: string; searchQuery?: string; onSearchChange?: (v: string) => void; className?: string; readOnly?: boolean }> = ({ value, options, onChange, placeholder, searchQuery, onSearchChange, className, readOnly }) => {
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-popover border border-border rounded-xl shadow-2xl z-50 max-h-[200px] overflow-y-auto">
          {filtered.map(o => (
            <button key={o.value} onMouseDown={() => { onChange(o.value); setOpen(false); setSearch(''); }}
              className="w-full px-4 py-2.5 text-xs hover:bg-muted text-left transition-colors">{o.label}</button>
          ))}
        </div>
      )}
    </div>
...
export const FingerprintSettings = ({ config, setConfig, presetId, activeStep, setActiveStep, isEditMode }: {
  config: FingerprintConfig; setConfig: React.Dispatch<React.SetStateAction<FingerprintConfig>>;
  presetId?: string | null; activeStep: number; setActiveStep: (step: number) => void; isEditMode: boolean;
}) => {
  ...
  const DNT_OPTS = [{ label: '0 (Bình thường)', value: '0' },{ label: '1 (Không theo dõi)', value: '1' }];
  ...
  return (
    <div className="max-w-4xl mx-auto pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-4">
      {/* Step 01 */}
      <FormSection title="01. User Agent" description="Định dạng cốt lõi xác định danh tính thiết bị." icon={Globe} isUnlocked={isEditMode || activeStep >= 1}>
        <FormItem label="User Agent String" description="Chuỗi định danh trình duyệt.">
          <NativeCombobox value={config.ua} options={USER_AGENTS} searchQuery={uaSearchQuery} onSearchChange={setUaSearchQuery}
            onChange={(val) => { const p = parseUserAgent(val); setConfig(prev => ({ ...prev, ua: val, ...p })); setUaSearchQuery(''); }} placeholder="Tìm kiếm User Agent..." />
        </FormItem>
        {!isEditMode && activeStep === 1 && <StepNavigation step={1} onPrev={() => {}} onNext={() => setActiveStep(2)} />}
      </FormSection>

      {/* Step 02 */}
      <FormSection title="02. Platform & OS Version" description="Đồng bộ hóa hệ điều hành." icon={Monitor} isUnlocked={isEditMode || activeStep >= 2}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <FormItem label="Platform Version" description="Phiên bản OS.">
            <NativeCombobox value={config.osVersion} options={[{ label: '10.0.0', value: '10.0.0' },{ label: '14.5.0', value: '14.5.0' },{ label: '6.5.0', value: '6.5.0' },{ label: '14.0.0', value: '14.0.0' },{ label: '17.5.1', value: '17.5.1' }]} onChange={(v) => setConfig(p => ({ ...p, osVersion: v }))} />
          </FormItem>
        ...
        {!isEditMode && activeStep === 2 && <StepNavigation step={2} onPrev={() => setActiveStep(1)} onNext={() => setActiveStep(3)} />}
      </FormSection>

      {/* Step 03 */}
      <FormSection title="03. Hardware Properties" description="Thông số CPU và RAM." icon={Activity} isUnlocked={isEditMode || activeStep >= 3}>
        ...
        {!isEditMode && activeStep === 3 && <StepNavigation step={3} onPrev={() => setActiveStep(2)} onNext={() => setActiveStep(4)} />}
      </FormSection>

      {/* Step 04 */}
      <FormSection title="04. Screen Properties" description="Kích thước hiển thị." icon={Monitor} isUnlocked={isEditMode || activeStep >= 4}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormItem label="Screen Resolution" description="Độ phân giải.">
            <NativeCombobox value={`${config.width}x${config.height}`} options={RES_OPTS} onChange={(v) => { const [w,h] = v.split('x').map(Number); setConfig(p => ({ ...p, width: w, height: h, availWidth: w, availHeight: h-40, outerWidth: w, outerHeight: h-40, innerWidth: Math.max(0,w-16), innerHeight: Math.max(0,h-112) })); }} />
          </FormItem>
        ...
        {!isEditMode && activeStep === 4 && <StepNavigation step={4} onPrev={() => setActiveStep(3)} onNext={() => setActiveStep(5)} />}
      </FormSection>

      {/* Step 05 */}
      <FormSection title="05. Window Properties" description="Kích thước cửa sổ." icon={Layout} isUnlocked={isEditMode || activeStep >= 5}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <FormItem label="Outer Resolution" description="Toàn bộ cửa sổ.">
            <NativeCombobox value={`${config.outerWidth}x${config.outerHeight}`} options={RES_OPTS} onChange={(v) => { const [w,h] = v.split('x').map(Number); setConfig(p => ({ ...p, outerWidth: w, outerHeight: h, innerWidth: Math.max(0,w-16), innerHeight: Math.max(0,h-72) })); }} />
          </FormItem>
        ...
        {!isEditMode && activeStep === 5 && <StepNavigation step={5} onPrev={() => setActiveStep(4)} onNext={() => setActiveStep(6)} />}
      </FormSection>

      {/* Step 06 */}
      <FormSection title="06. Language & Locale" description="Ngôn ngữ trình duyệt." icon={Globe} isUnlocked={isEditMode || activeStep >= 6}>
        <FormItem label="Primary Language" description="Ngôn ngữ chính." action={
          <button onClick={() => setConfig(p => ({ ...p, autoIpLanguage: !p.autoIpLanguage }))}
            className={cn('flex items-center gap-2 px-3.5 h-8 rounded-t-xl border border-b-0 text-[9px] font-black uppercase tracking-widest transition-all',
              config.autoIpLanguage ? 'bg-primary/20 text-primary border-primary/30' : 'bg-muted/5 text-muted-foreground/40 border-border/10')}>
            <Globe className="w-2.5 h-2.5" />{config.autoIpLanguage ? 'ON AUTO IP' : 'AUTO IP'}
          </button>}>
          {config.autoIpLanguage ? <input type="text" readOnly value="Tự động dựa vào IP" className={inputReadOnly} /> :
            <NativeCombobox value={config.primaryLanguage} options={LANG_OPTS} onChange={(v) => setConfig(p => ({ ...p, primaryLanguage: v }))} />}
        </FormItem>
        <FormItem label="Languages (JSON)" description="Danh sách ngôn ngữ.">
          {config.autoIpLanguage ? <input type="text" readOnly value="Tự động dựa vào IP" className={inputReadOnly} /> :
            <NativeCombobox value={config.languages} options={LANGS_OPTS} onChange={(v) => setConfig(p => ({ ...p, languages: v }))} />}
        </FormItem>
        {!isEditMode && activeStep === 6 && <StepNavigation step={6} onPrev={() => setActiveStep(5)} onNext={() => setActiveStep(7)} />}
      </FormSection>

      {/* Step 07 */}
      <FormSection icon={MapPin} title="07. Timezone & GPS" description="Vị trí địa lý." isUnlocked={isEditMode || activeStep >= 7}>
        <FormItem label="Timezone" description="Múi giờ." action={
          <button onClick={() => setConfig(p => ({ ...p, autoIpTimezone: !p.autoIpTimezone }))}
            className={cn('flex items-center gap-2 px-3.5 h-8 rounded-t-xl border border-b-0 text-[9px] font-black uppercase tracking-widest transition-all',
              config.autoIpTimezone ? 'bg-primary/20 text-primary border-primary/30' : 'bg-muted/5 text-muted-foreground/40 border-border/10')}>
            <Globe className="w-2.5 h-2.5" />{config.autoIpTimezone ? 'ON AUTO IP' : 'AUTO IP'}
          </button>}>
          <input type="text" readOnly value={config.autoIpTimezone ? 'Tự động dựa vào IP' : config.timezone} className={inputReadOnly} />
        </FormItem>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FormItem label="Longitude" description="Kinh độ." action={
            <button onClick={() => setConfig(p => ({ ...p, autoIpGps: !p.autoIpGps }))}
              className={cn('flex items-center gap-2 px-3.5 h-8 rounded-t-xl border border-b-0 text-[9px] font-black uppercase tracking-widest transition-all',
                config.autoIpGps ? 'bg-primary/20 text-primary border-primary/30' : 'bg-muted/5 text-muted-foreground/40 border-border/10')}>
              <MapPin className="w-2.5 h-2.5" />{config.autoIpGps ? 'ON AUTO GPS' : 'GPS'}
            </button>}>
            <input type="text" readOnly={config.autoIpGps} value={config.autoIpGps ? 'Auto IP' : String(config.longitude)}
              onChange={(e) => setConfig(p => ({ ...p, longitude: parseFloat(e.target.value) || 0 }))}
              className={cn(inputClass, config.autoIpGps && 'opacity-60 bg-muted/20 cursor-not-allowed')} />
          </FormItem>
          <FormItem label="Latitude" description="Vĩ độ.">
            <input type="text" readOnly={config.autoIpGps} value={config.autoIpGps ? 'Auto IP' : String(config.latitude)}
              onChange={(e) => setConfig(p => ({ ...p, latitude: parseFloat(e.target.value) || 0 }))}
              className={cn(inputClass, config.autoIpGps && 'opacity-60 bg-muted/20 cursor-not-allowed')} />
          </FormItem>
        ...
        {!isEditMode && activeStep === 7 && <StepNavigation step={7} onPrev={() => setActiveStep(6)} onNext={() => setActiveStep(8)} />}
      </FormSection>

      {/* Step 08 */}
      <FormSection title="08. Canvas & Media" description="Đồ họa và âm thanh." icon={Monitor} isUnlocked={isEditMode || activeStep >= 8}>
        ...
        {!isEditMode && activeStep === 8 && <StepNavigation step={8} onPrev={() => setActiveStep(7)} onNext={() => setActiveStep(9)} />}
      </FormSection>

      {/* Step 09 */}
      <FormSection title="09. WebGL Properties" description="Tham số đồ họa WebGL." icon={Monitor} isUnlocked={isEditMode || activeStep >= 9}>
        <div className="grid grid-cols-1 gap-8">
          <FormItem label="WebGL Vendor" description="GPU Vendor."><NativeCombobox value={config.webglVendor} options={[{ label: 'Google Inc. (NVIDIA)', value: 'Google Inc. (NVIDIA)' },{ label: 'Google Inc. (Intel)', value: 'Google Inc. (Intel)' },{ label: 'Apple Inc.', value: 'Apple Inc.' }]} onChange={(v) => setConfig(p => ({ ...p, webglVendor: v }))} /></FormItem>
          <FormItem label="WebGL Renderer" description="Card đồ họa."><NativeCombobox value={config.webglRenderer} options={WEBGL_OPTS} onChange={(v) => setConfig(p => ({ ...p, webglRenderer: v }))} /></FormItem>
          <FormItem label="WebGL Parameters (JSON)" description="Tham số mở rộng.">
            <NativeCombobox value={config.webglParameters} options={[{ label: 'High-End', value: '{"MAX_TEXTURE_SIZE":16384,"MAX_VIEWPORT_DIMS":[16384,16384]}' },{ label: 'Mid-Range', value: '{"MAX_TEXTURE_SIZE":8192,"MAX_VIEWPORT_DIMS":[8192,8192]}' }]} onChange={(v) => setConfig(p => ({ ...p, webglParameters: v }))} />
          </FormItem>
        </div>
        {!isEditMode && activeStep === 9 && <StepNavigation step={9} onPrev={() => setActiveStep(8)} onNext={() => setActiveStep(10)} />}
      </FormSection>

      {/* Step 10 */}
      <FormSection title="10. Browser Metadata" description="Thông số bảo mật." icon={Globe} isUnlocked={isEditMode || activeStep >= 10}>
        <div className="grid grid-cols-1 gap-8">
        ...
        {!isEditMode && activeStep === 10 && <StepNavigation step={10} onPrev={() => setActiveStep(9)} onNext={() => {}} isLast />}
      </FormSection>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md mx-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-border/50">
              <h3 className="text-sm font-black uppercase tracking-[0.25em] text-foreground">Lưu cấu hình vân tay</h3>
              <p className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest mt-1">Xác định danh tính cho bộ hồ sơ của bạn</p>
            </div>
            <div className="px-6 py-4 space-y-8">
              <div className="space-y-3">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-[12px] font-bold uppercase tracking-wider text-muted-foreground/70">Tên</label>
                  <button onClick={() => { const ua = USER_AGENTS.find(u => u.value === config.ua); if (ua) setConfig(p => ({ ...p, profileName: ua.label })); }}
                    className="px-3.5 h-7 rounded-t-xl bg-primary/20 text-primary text-[9px] font-black uppercase tracking-widest border border-primary/30 border-b-0">Dùng UA làm tên</button>
                </div>
                <input type="text" placeholder="Ví dụ: Chrome Windows 11" value={config.profileName || ''}
                  onChange={(e) => setConfig(p => ({ ...p, profileName: e.target.value }))}
              ...
              <div className="space-y-3">
                <label className="text-[12px] font-bold uppercase tracking-wider text-muted-foreground/70">Mô tả</label>
                <textarea placeholder="Nhập mô tả ngắn gọn..." value={config.profileDescription || ''}
                  onChange={(e) => setConfig(p => ({ ...p, profileDescription: e.target.value }))}
            ...
            <div className="px-6 py-4 border-t border-border/50 flex gap-4">
              <button onClick={() => setIsSaveDrawerOpen(false)} className="flex-1 h-12 rounded-xl bg-muted/10 text-muted-foreground font-black text-[10px] uppercase tracking-widest hover:bg-muted/20 transition-all border border-border/10">Hủy</button>
              <button onClick={async () => {
                ...
                } catch (error) { console.error('Failed to save fingerprint:', error); alert('Lỗi khi lưu cấu hình.'); }
                ...
                {saveLoading ? <div className="w-5 h-5 border-3 border-primary border-t-transparent rounded-full animate-spin" /> : <><CheckCircle2 className="w-5 h-5" />Hoàn tất & Lưu</>}
              </button>
            </div>
          </div>
        </div>
