// src/renderer/src/presentation/pages/PeopleManager/components/DatabaseModal.tsx
import React, { useState, useEffect } from 'react'
import { motion } from 'framer-motion'
import { Database, FolderOpen, Plus, AlertCircle, Clock, X, Users } from 'lucide-react'
...
const DatabaseModal: React.FC<DatabaseModalProps> = ({ onDatabaseSelected, className }) => {
  ...
  useEffect(() => {
    const loadRecentDatabase = async () => {
      try {
        const lastDatabase = await peopleService.getLastDatabase()
        if (lastDatabase) {
          ...
          if (exists) {
            ...
            await peopleService.clearLastDatabase()
          }
        }
      } catch (error) {
  ...
  const handleCreateNew = async () => {
    try {
      ...
      const path = await peopleService.createNewDatabase()
      if (path) {
        onDatabaseSelected()
      }
    } catch (error) {
  ...
  const handleOpenExisting = async () => {
    try {
      ...
      const path = await peopleService.openExistingDatabase()
      if (path) {
        onDatabaseSelected()
      }
    } catch (error) {
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-6 text-white">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl font-bold">People Manager Database</h2>
          </div>
          <p className="text-blue-100 text-sm">
            Select a database file to get started with managing your people database
          </p>
        </div>
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 dark:text-amber-400">
                  Your people data is stored locally in an encrypted SQLite database. Keep your
