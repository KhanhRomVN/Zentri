(function () {
  ...
  const queueEvent = (timer, delay = 0) => {
    timer.stop();
    return new Promise((resolve) => setTimeout(() => resolve(timer.start()), delay)).catch(
      (e) => {},
    );
  };
...
`;
  ...
  const getPluginLies = (plugins, mimeTypes) => {
    ...
    const pluginsOwnPropertyNames = Object.getOwnPropertyNames(plugins).filter((name) =>
      isNaN(+name),
    );
    const mimeTypesOwnPropertyNames = Object.getOwnPropertyNames(mimeTypes).filter((name) =>
      isNaN(+name),
    );
    // cast to array
  ...
  const getWebGLRendererConfidence = (x) => {
    ...
    const confidence =
      valid && !gibbers.length ? 'high' : valid && gibbers.length ? 'moderate' : 'low';
  ...
  async function getBestWorkerScope() {
    try {
      ...
      // detect lies
      const { system, userAgent, userAgentData, platform, deviceMemory, hardwareConcurrency } =
        workerScope || {};
      // navigator lies
  ...
  // instance id
  const instanceId =
    String.fromCharCode(Math.random() * 26 + 97) + Math.random().toString(36).slice(-7);
  ...
  async function getOfflineAudioContext() {
    try {
      ...
      const { floatFrequencyData, floatTimeDomainData, buffer, compressorGainReduction } =
        audioData || {};
      await queueEvent(timer);
      ...
      const getCopyFrom = (rand, buffer, copy) => {
        ...
        return [...new Set([...buffer.getChannelData(0), ...copy, ...attack])].filter(
          (x) => x !== 0,
        );
      };
  ...
  function getCSSMedia() {
    ...
    try {
      ...
      const matchMediaCSS = {
        ['prefers-reduced-motion']: win.matchMedia('(prefers-reduced-motion: no-preference)')
          .matches
          ? 'no-preference'
        ...
        pointer: win.matchMedia('(pointer: fine)').matches
          ...
          : win.matchMedia('(pointer: coarse)').matches
            ...
            : win.matchMedia('(pointer: none)').matches
              ? 'none'
              : undefined,
        ['device-aspect-ratio']: win.matchMedia(`(device-aspect-ratio: ${deviceAspectRatio})`)
          .matches
          ? deviceAspectRatio
          : undefined,
      ...
      !matchMediaCSS || !Object.keys(matchMediaCSS).filter((key) => !!matchMediaCSS[key]).length
        ...
        : modal(
            ...
            `<strong>matchMedia</strong><br><br>${Object.keys(matchMediaCSS)
              .map((key) => `${key}: ${matchMediaCSS[key] || HTMLNote.UNSUPPORTED}`)
              .join('<br>')}`,
  ...
  const getJSCoreFeatures = (win) => {
    ...
    try {
      ...
      const features = globalObjects.reduce((acc, name) => {
        ...
        const uniques = [
          ...new Set(
            [...descriptorKeys, ...descriptorProtoKeys].filter((key) => !ignore.includes(key)),
          ),
        ];
  ...
  async function getEngineFeatures({ cssComputed, navigatorComputed, windowFeaturesComputed }) {
    try {
      ...
      const { liedVersion: liedWindow, distance: distanceWindow } = getVersionLie(
        vReport,
        windowVersion,
      );
      if (liedCSS) {
    ...
    const getPlatformVersion = (fonts) => {
      ...
      const getMacOS = ({ fonts, fontMap }) => {
        const fontVersion = {
          ...
          ['10.9']:
            fontMap['10.9'].filter((x) => fonts.includes(x)).length == fontMap['10.9'].length,
        };
        const hash =
          '' +
          Object.keys(fontVersion)
      ...
      return (
        getWindows({ fonts, fontMap: WindowsFonts }) || getMacOS({ fonts, fontMap: MacOSFonts })
      );
    };
  ...
  function fontsHTML(fp) {
	...
	<div class="relative col-six${lied ? ' rejected' : ''}">
		...
		<div class="help" title="FontFace.load()">load (${fontFaceLoadFonts ? count(fontFaceLoadFonts) : '0'}/${'' + FONT_LIST.length}): ${
  ...
  function mathsHTML(fp) {
	...
	<div class="relative col-six${lied ? ' rejected' : ''}">
		...
		<div>results: ${
      !data ? HTMLNote.Blocked : modal('creep-maths', header + results.join('<br>'))
    }</div>
	</div>
	`;
  }
      ...
      const data = {
        ...
        deviceMemory: attempt(() => {
          ...
          if (!trusted[deviceMemory]) {
            sendToTrash(
              'deviceMemory',
              `${deviceMemory} is not a valid value [0.25, 0.5, 1, 2, 4, 8, 16, 32]`,
            );
          }
  ...
  async function getResistance() {
    try {
      ...
      if (IS_GECKO && protection) {
        ...
        const torBrowser =
          featureKeys.filter((key) => targetSet.has(key) && !features[key]).length ==
          targetSet.size;
  ...
  function voicesHTML(fp) {
    ...
    const {
      voices: { $hash, local, remote, languages, defaultVoiceName, defaultVoiceLang, lied },
    } = fp;
    const icon = {
      Linux: '<span class="icon linux"></span>',
      Apple: '<span class="icon apple"></span>',
    ...
    const systemVoice =
      Object.keys(system).find((key) => local.find((voice) => voice.includes(key))) || '';
    return `
	<div class="relative col-four${lied ? ' rejected' : ''}">
      ...
      !languages || !languages.length
        ...
        : languages.length == 1
          ? languages[0]
          : modal('creep-voices-languages', languages.join('<br>'), hashMini(languages))
    }</div>
  ...
  async function getStatus() {
    const [batteryInfo, quotaA, quotaB, scriptSize, stackSize, timingRes, clientLitter] =
      await Promise.all([
        getBattery(),
        getStorage(),
        getStorage(),
  ...
  function getTimezone() {
    ...
    const decryptLocation = ({ year, timeZone }) => {
      ...
      const filter = (cities) =>
        cities.filter((city) => system == getTimezoneOffsetHistory({ year, city }));
      // get city region set
  ...
  async function getCanvasWebgl() {
    ...
    const draw = (gl) => {
      const isSafari15AndAbove =
        'BigInt64Array' in window && IS_WEBKIT && !/(Cr|Fx)iOS/.test(navigator.userAgent);
    ...
    try {
      ...
      const capabilities = [
        ...
        -638494755, -671082546, -677558160, -999987216, 1099536, 1099644, 1147714426, 1197075,
        1229835, 1508998, 1509050, 1610618841, 184555483, 2146590728, 2147305224, 2147361749,
        2147440438, 2147475085, 2147479181, 21667, 349912, 351513, 83625, 998804992, 998911268,
        999148597, 999156922,
      ];
      ...
      const hasSusGpu =
        webglBrandCapabilities && !brandCapabilities.includes(webglBrandCapabilities);
  ...
  function webglHTML(fp) {
    ...
    const { $hash, dataURI, dataURI2, pixels, pixels2, lied, extensions, parameters, gpu } =
      data || {};
    const {
      parts,
      warnings,
  ...
  !(async function () {
    ...
    ] = await Promise.all([fingerprint().catch((error) => console.error(error)) || {}]);
    if (!fp) {
    ...
    const creep = {
      ...
      fonts:
        !fp.fonts || fp.fonts.lied || LowerEntropy.FONTS ? undefined : fp.fonts.fontFaceLoadFonts,
        ...
        Promise.all([getWebRTCData(), getWebRTCDevices(), getStatus()])
          .then(async (data) => {
            ...
            patch(
              ...
              html` <div class="flex-grid visitor-info">${webrtcHTML(webRTC, mediaDevices)}</div> `,
            );
            patch(
          ...
          html`
            <div id="fuzzy-fingerprint">
              <div class="ellipsis-all fuzzy-fp">
                Fuzzy: <span class="unblurred">${fuzzyFingerprint}</span>
              </div>
            </div>
          `,
        );
